    ...
    def _draw_lines(self, grid_state, info):
        ...
        for b in info['bodies']:
            ...
            for b_neighbor_index in b[8]:
                ...
                start_ind = int(b[7]) * 5
                # We consider the joint the "halfway" point for values
                self._dda_draw(
                    grid_state,
                    (grid_x, grid_y),
                    (shared_joint_grid_x, shared_joint_grid_y),
                    np.array(b[2:7]),
                    np.array(b_neighbor[2:7]) / 2,
        ...
        for j in info['joints']:
            ...
            for j_neighbor_index in j[8]:
                ...
                self._dda_draw(
                    grid_state,
                    (A_grid_x, A_grid_y),
                    (shared_body_grid_x, shared_body_grid_y),
                    np.array(j[4:6]),
                    np.array(j_neighbor[4:6]) / 2,
