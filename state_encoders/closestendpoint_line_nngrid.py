    ...
    def _dda_draw(self, grid_state, start, end, start_vals, end_vals, channels):
        ...
        if start_x == end_x:
            ...
            for y in range(start_y, end_y+1):
                # Fill in cells with value of closest endpoint
                if y < (start_y + end_y + 1):
                    grid_state[start_channel:end_channel, round(x), y] = Variable(torch.from_numpy(start_vals.astype(float)))
                else:
                    grid_state[start_channel:end_channel, round(x), y] = Variable(torch.from_numpy(end_vals.astype(float)))
            return
        ...
        if abs(m) > 1:
            ...
            for x in range(start_x, end_x+1):
                if x < (start_x + end_x + 1):
                    grid_state[start_channel:end_channel, x, round(y)] = Variable(torch.from_numpy(start_vals.astype(float)))
                else:
                    grid_state[start_channel:end_channel, x, round(y)] = Variable(torch.from_numpy(end_vals.astype(float)))
                y = y + float(m)
        ...
        for j in info['joints']:
            ...
            for j_neighbor_index in j[8]:
                ...
                A_neighbor_grid_x, A_neighbor_grid_y = self._coords_to_grid(
                    A_neighbor_pos_x, A_neighbor_pos_y, zero_x, zero_y)
