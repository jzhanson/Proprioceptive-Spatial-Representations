...
import random
import numpy as np
# TODO(josh): Make python path work across packages. Dots in front are to make a3g_main run
...
class RandomizeBodies:
    def __init__(self, body_type, args):
        ...
        self.args = args
        # None directory used for randomization in TrueRandomJSONWalker
        if self.args['directory'] is not None and not os.path.exists(self.args['directory']):
        ...
        for i in range(self.args['num_bodies']):
            ...
            if self.body_type == 'BipedalWalker':
                ...
                if self.args['hull_segment'] == 'center':
                    ...
                    gen_args['hull_segment'] = random.choice(np.arange(-0.5,
                    ...
                    gen_args['hull_segment'] = random.choice(np.arange(
