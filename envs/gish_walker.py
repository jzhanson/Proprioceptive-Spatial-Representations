...
import Box2D
from Box2D.b2 import (edgeShape, circleShape, fixtureDef, polygonShape, revoluteJointDef, distanceJointDef, weldJointDef, contactListener)
...
class ContactDetector(contactListener):
    ...
    def BeginContact(self, contact):
        ...
        for body in self.env.bodies:
            if body in [contact.fixtureA.body, contact.fixtureB.body]:
                body.ground_contact = True
    def EndContact(self, contact):
        for body in self.env.bodies:
            if body in [contact.fixtureA.body, contact.fixtureB.body]:
                body.ground_contact = False

class GishWalker(gym.Env):
    metadata = {
    ...
    def step(self, action):
        ...
        if control_speed:
            ...
            for a in range(len(action)):
                self.motors[a].motorSpeed    = float(SPEED_HIP * np.sign(action[a]))
                self.motors[a].maxMotorSpeed = float(MOTORS_TORQUE * np.clip(np.abs(action[a]), 0, 1))
...
class GishWalkerHardcore(GishWalker):
    hardcore = True
